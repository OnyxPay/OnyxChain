...
func encodeID(id []byte) ([]byte, error) {
	...
	if length == 0 || length > 255 {
		return nil, errors.New("encode ONX ID error: invalid ID length")
	}
	//enc := []byte{byte(length)}
	enc := append(utils.OnxIDContractAddress[:], byte(length))
	enc = append(enc, id...)
	return enc, nil
}

func decodeID(data []byte) ([]byte, error) {
	prefix := len(utils.OnxIDContractAddress)
	size := len(data)
	if size < prefix || size != int(data[prefix])+1+prefix {
		return nil, errors.New("decode ONX ID error: invalid data length")
	}
