...
func (self *StateStore) CheckStorage() error {
	...
	prefix := append([]byte{byte(scom.ST_STORAGE)}, utils.OnxIDContractAddress[:]...) //prefix of new storage key
	flag := append(prefix, onxid.FIELD_VERSION)
	val, err := db.Get(flag)
	if err == nil {
		item := &states.StorageItem{}
		buf := bytes.NewBuffer(val)
		err := item.Deserialize(buf)
		if err == nil && item.Value[0] == onxid.FLAG_VERSION {
			return nil
		} else if err == nil {
			return errors.New("check onxid storage: invalid version flag")
		} else {
			return err
		}
	}
	...
	tag.Value = []byte{onxid.FLAG_VERSION}
	buf := bytes.NewBuffer(nil)
	tag.Serialize(buf)
